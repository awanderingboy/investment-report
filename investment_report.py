...
0. 📌 오늘 한 장 요약 (핵심 지표 요약표 — 보고서 맨 위)
0.5. 💼 내 포트폴리오 현황
0.6. 📊 보유 종목 한눈에 보기
0.7. ✅ 오늘의 실행표 (우선순위 5종목)
0.8. 🕒 데이터 기준
1. 오늘의 핵심 결론 (10줄)
2. 데이터 신뢰도 체크
3. 시장/거시 환경
...
보고서 길이 압축 원칙:
- 0~0.8 섹션(요약/현황): 표 중심, 각 표 10줄 이내. 수치는 [포트폴리오 사전 계산값] 그대로 사용.
- 1~9 섹션(분석/실행): 핵심 수치+판단 위주, 산문 설명 2줄 이내로 압축.
- 10~16.7 섹션(검증/결론): 표 중심, 이미 앞 섹션에서 언급한 내용 반복 금지.

[0. 📌 오늘 한 장 요약 — 보고서 맨 위에 반드시 출력]
보고서 전체를 한 눈에 파악할 수 있는 핵심 지표 요약표다. 실제 수치를 채워라. 빈칸/대략 표현 금지.
...
| 전략 모드 | 공격/중립/방어 |
| 신규 매수 | 가능/금지/조건부 |
| 오늘 1순위 실행 | (종목명: 구체적 행동 1줄) |
| 오늘 반드시 피할 것 | (1줄) |
| 가격 조건 발동 종목 | (종목명 또는 없음) |
| TDE A/B 후보 수 | N개 |
| 현금 비중 | X% |
| 고위험(유형③) 비중 | X% |
| 모델 정확도(전일) | X% |
...
작성 원칙:
- 가격 조건 발동 종목이 있으면 반드시 종목명을 명시해라.
- TDE A/B 후보 0개이면 "0개 (신규 목돈 매수 금지)"로 표시해라.

[0.5. 💼 내 포트폴리오 현황 — 반드시 출력]
[포트폴리오 사전 계산값]에 제공된 수치를 그대로 사용해라. 직접 계산 금지.
...
[0.6. 📊 보유 종목 한눈에 보기 — 반드시 출력]
모든 보유 종목을 아래 표 형식으로 출력해라. 현재가와 수익률은 [포트폴리오 사전 계산값] 기준.

📊 보유 종목 한눈에 보기

| 종목 | 유형 | 현재가 | 수익률 | 비중 | 오늘 상태 |
|------|------|--------|--------|------|-----------|

오늘 상태 기준: 보유(현재 전략 유지) / 감시(가격/조건 모니터링) / 축소검토(조건부/즉시) / 발동(가격 조건 발동)

[0.7. ✅ 오늘의 실행표 — 반드시 출력]
가장 중요한 5개 종목의 오늘 실행 가이드를 우선순위 순으로 출력해라.
실행 우선순위: 가격 조건 TRIGGERED 종목 > 셀트리온제약 > RGTI > BEAM > GOOGL > 네이버

✅ 오늘의 실행표

| 순위 | 종목 | 오늘 실행 | 조건부 실행 | 금지 행동 |
|------|------|-----------|-------------|-----------|

작성 원칙:
- 가격 조건 TRIGGERED 종목은 "오늘 실행" 칸에 즉시 재검토 행동을 명시해라.
- TDE 신규 매수 금지 상태에서는 "오늘 실행" 칸에 신규 매수를 절대 쓰지 마라.
- 각 칸은 1~2줄 내외로 간결하게 작성해라.

[0.8. 🕒 데이터 기준 — 반드시 출력]
각 데이터의 수집 시점과 신뢰도를 명확히 표시해라. 미국 주식의 실시간/지연 여부를 반드시 구분해라.

🕒 데이터 기준

| 데이터 | 기준 시점 | 비고 |
|--------|-----------|------|
| 미국 주식 | 장중 실시간/전일 종가/지연 중 하나 명시 | 실시간 여부 표시 |
| 한국 주식 | 전일 종가 기준 | 당일 장중 수집 불가 |
| 환율 | 보고서 생성 시점 실시간 | |
| 거시지표 | 수집 시점 명시 | 단일 소스 여부 표시 |
| 뉴스 | 최근 7일 기준 | A등급 뉴스 존재 시 명시 |

미국 주식 구분 원칙:
- 장중 수집: "실시간 (수집 시각 KST)"로 표시
- 전일 종가: "전일 종가 (YYYY-MM-DD)"로 표시
- 지연 데이터: "지연 (Xh 지연)"으로 표시
...
중요: 보유 종목만 포함해라.
- portfolio에 없는 종목(JOBY, 삼성전자, SK하이닉스 등 비보유 종목)은 이 표에 포함하지 마라.
- 비보유 종목 점수 분석은 7번 🔎 신규/관찰 후보 섹션에서만 허용한다.
- 시장 참고 문구(예: 한국 반도체 과열)에서 짧게 언급하는 것은 허용하나, 점수표 행으로 추가하지 마라.
...
이 섹션은 두 파트로 구성된다:
(A) 관찰 후보: 조건 충족 시 진입 검토 가능한 종목 (C등급 포함)
(B) 제외 후보: 현재 D등급이나 조건 변화 시 재검토 가능한 종목

[A. 관찰 후보 — 실시간 수집 가능 종목 중 C등급 이상]

| 종목 | 가격 위치 | 뉴스 등급 | 차트 점수 | 거래량 점수 | 펀더 점수 | 밸류 점수 | 리스크 보상비 | 포트폴리오 적합성 | 등급 | 진입 조건 |
|---|---|---|---|---|---|---|---|---|---|---|

[B. 제외 후보 — 현재 D등급, 조건 충족 시 재검토]

| 종목 | 현재 등급 | 제외 이유 | B등급 전환 조건 |
|---|---|---|---|

후보 종목 선정 규칙:
- 실시간 가격 수집 가능한 종목만 포함
- 수집 불가능한 종목은 "수동 확인 필요"로 별도 표시 (표 본문 제외)
- 기존 보유 종목 중 추가매수 후보는 신규 진입과 추매를 구분해서 표시
- 고위험 테마주(유형③)는 A/B 금지 사유를 표에 명시해라
- 신규 매수 금지 상태여도 두 파트 모두 출력해라

출력 예시 (관찰 후보):
| GOOGL | 390달러, 370달러 대기 | B | 6/10 | 5/10 | 9/10 | 6/10 | 410목표/370손절 | 적합 | C | 370달러 이하에서 B 가능 |
| VOO | RSI 73 과열 | B | 5/10 | 6/10 | 8/10 | 5/10 | 과열 구간 | 적합 | C | RSI 60 이하 전까지 추가 일시매수 보류 |

출력 예시 (제외 후보):
| RGTI | D | 적자/고위험 비중 초과/신뢰도 낮음 | 고위험 비중 10% 이하 + 정확도 50% 이상 회복 시 재검토 |
...
8.5번 섹션은 상단 "✅ 오늘의 실행표(0.7번)"와 동일한 표를 반복 출력하지 마라.
0.7번에서 다루지 않은 추가 관리 종목이 있을 때만 1~2줄로 보완해라.
추가 사항이 없으면 아래 한 줄만 출력해라:

8.5 실제 매매 실행표: 상단 "✅ 오늘의 실행표" 기준 — 추가 사항 없음.

금지: 0.7번과 동일한 종목·동일한 실행 내용을 여기서 다시 반복 출력하지 마라.
...
[14.5. 🧠 전략 모드 판단 — 보고서 등급 다음에 반드시 출력]

14번 보고서 등급 섹션 다음에 아래 표를 출력해라.

🧠 전략 모드 판단

| 기준 | 현재 상태 | 판단 |
|------|-----------|------|
| 모델 정확도(전일) | X% | 정상(≥70%) / 저하(50-70%) / 경보(<50%) |
| TDE A/B 후보 수 | N개 | 공격(≥2) / 중립(1) / 방어(0) |
| 고위험(유형③) 비중 | X% | 정상(<10%) / 경고(10-15%) / 과다(>15%) |
| 셀트리온제약 비중 | X% | 정상(<25%) / 집중위험(≥25%) |
| 현금 비중 | X% | 충분(≥10%) / 부족(5-10%) / 위험(<5%) |
| 최종 전략 모드 | | 공격 / 중립 / 방어 |

전략 모드 결정 규칙:
- 공격: 모델 정확도 70%+, TDE A/B 2개 이상, 고위험 정상, 현금 충분 → 모든 조건 충족 시
- 중립: 공격 조건 일부 미충족 (정확도 50-70% 또는 TDE A/B 1개 또는 현금 부족)
- 방어: 정확도 50% 미만, 또는 고위험 과다(>15%), 또는 현금 위험(<5%) 중 하나라도 해당 시
- 셀트리온제약 비중 25% 이상이면 최소 중립, 공격 금지
...
본 보고서의 실행 가능 판단은 실시간 가격·뉴스·차트·거래량 교차검증 결과를 기준으로 한다.

📌 최종 매매 결론 (3줄 요약)

1) 신규 목돈 매수: [금지/가능/조건부] — 이유 1줄 (정확도, 뉴스, 차트, 포트폴리오 리스크)
2) 기존 자동 적립: [VOO/GOOGL/FCX 유지 여부] — 조건 1줄
3) 오늘 1순위: [종목명] — 구체적 행동 1줄

수익형 모드: ON/OFF (10개 조건 중 N개 충족)
조건부 축소 대상: [셀트리온제약 / RGTI / BEAM 등 조건 포함]
PLTR 상태: 보유 유지 (잔량, 추가매수 금지) 또는 조건부 축소 (120달러 이탈 시)

작성 원칙:
- 0.7 오늘의 실행표에서 이미 표로 정리된 내용을 다시 긴 표로 반복하지 마라.
- 신규 매수 금지 이유는 정확도만이 아니라 뉴스/차트/포트폴리오 리스크도 1줄에 함께 써라.
- 신규 매수 A가 가능하면 반드시 진입 조건(가격, 수량, 손절가)을 추가해라.
- A등급 후보가 없으면 "A등급 후보 없음 — [어떤 조건이면 B/A 가능]"을 명시해라.
- PLTR을 RGTI와 같은 "손실 제한 감시"로 묶지 마라.
- 즉시 매도는 실시간 가격 조건 또는 A등급 악재가 있을 때만 표시해라.

최종 결론과 오늘의 실행표(0.7번)가 충돌하면 실패로 간주해라.
...
- 오늘의 실행표(0.7번)와 최종 매매 결론이 충돌하는 경우
- 0.7 오늘의 실행표와 8.5 섹션에 동일 종목·동일 실행 내용이 중복 출력되는 경우

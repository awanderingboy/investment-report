...
평가 요소: 데이터 신뢰도 체크 / 가격 조건 검증 / 패턴 조정 표시 / 포트폴리오 비중 진단 / 사용자 개인 전략 반영 / 자기검열 구조
...
보고서 구조 품질: 91점
모델 예측 신뢰도: 70점
실제 매매 활용도: 신규 매수 부적합 / 보유 리스크 관리용 적합
...
등급 제한 규칙:
- 자기검열 9/10 미만이면 보고서 구조 품질 최대 80점
- 점수 산정 근거 항목별 없으면 보고서 구조 품질 최대 75점
- 가격 조건 오류가 있으면 보고서 구조 품질 최대 70점
- 사용자 개인 전략 위반이 있으면 보고서 구조 품질 최대 75점
- 전일 정확도 30% 미만이면 보고서 구조 품질 최대 80점으로 제한
- 전일 정확도 10% 미만이면 보고서 구조 품질 최대 70점으로 제한
- 자기검열 최종이 5/10 이하이면 보고서 구조 품질 최대 75점으로 제한 (6/10은 해당 안 됨)
- 등급 제한이 적용된 경우, 제한 사유를 반드시 표시해라
- 보고서 구조가 좋아도 모델 예측 신뢰도가 낮으면 "실전 신규 매수 가능"으로 표시하지 마라
- 전일 정확도 30% 미만이면 신규 매수 보고서 등급은 자동으로 "부적합" 처리해라
- 보고서 구조 품질 90점 이상이라도 모델 예측 신뢰도 70점 미만이면 신규 매수는 금지 표시

판단 예시:
  전일 정확도 18% → 30% 미만이므로 보고서 구조 품질 최대 80점 제한. 10% 미만 아니므로 70점 제한 적용 안 됨.
  자기검열 최종 6/10 → 5/10 이하 아니므로 75점 제한 자동 적용 안 함.
  전일 정확도 18% → 신규 매수 보고서 등급 자동 "부적합"
...
| 가격 구간 | 의미 | 행동 |
|---|---|---|
| 22달러 종가 이탈 | 단기 회복 시나리오 훼손 | 절반 축소 검토 |
| 19~20달러 이탈 | 고위험 손실 확대 | 전량 재검토 |
| 24~25달러 | 현재 감시 구간 | 신규/추매 금지, 보유만 |
| 28~30달러 회복 | 반등 성공 구간 | 욕심 금지, 고위험 비중 10% 초과 시 일부 축소 우선 |
| 30달러 이상 안착 | 과열/수익 실현 구간 | 잔여 비중 축소 또는 원금 회수 검토 |
...
아래 중 하나라도 해당하면 신규 매수는 원칙적 금지로 표시해라:
- 전일 정확도 30% 미만
- 최근 5일 승률 40% 미만
- VOO 대비 초과성과 30% 미만
- korea_momentum_fail 최근 5일 5회 이상
- general_miss 최근 5일 10회 이상
- 고위험 옵션성 비중 10% 초과

신규 매수 차단 사유가 있으면 아래 형식으로 반드시 출력해라:
신규 매수 차단 사유:
- 전일 정확도 X% < 30%
- 최근 5일 승률 X% < 40%
- 한국 모멘텀 실패 X회
...
판단:
신규 매수 원칙적 금지. 기존 보유 리스크 관리와 손실 제한만 허용.
...
| 구분 | 판단 |
|---|---|
| 신규 매수 | 금지/가능/조건부 중 하나로 표시 |
| 기존 적립 | VOO/GOOGL/FCX만 유지 여부 표시 |
| 보유 유지 | 해당 종목 나열 |
| 손실 제한 감시 | 해당 종목 나열 |
| 조건부 축소 | 해당 종목 나열 |
| 즉시 매도 | 없음 또는 해당 종목 |

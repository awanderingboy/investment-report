...
3. 📰 실시간 뉴스/이슈 영향 분석
4. 📊 보유 종목별 상세 전략
5. 🎯 신규 수익 발굴 요약 (3줄 — 표 금지)
6. 📈 상승/하락 시나리오
7. ⚠️ 리스크 경고판
8. 🔍 데이터 신뢰도 검증
9. 📌 최종 실행 플랜
(코드 생성 섹션 자동 추가: 스크리너 TOP10, TDE, 가격 조건 — LLM 본문에 출력 금지)
...
[섹션 5. 🎯 신규 수익 발굴 요약]
신규 후보 상세표는 코드가 자동 생성하는 "스크리너 TOP 10" 섹션에서 제공된다.
LLM은 신규 후보 표·수치를 직접 생성하지 마라. 현재가·RR·손절가·목표가가 없는 표를 만들지 마라.
스크리너 결과를 아래 형식으로 3줄 이내로만 요약하라.
...
[섹션 6. 📈 상승/하락 시나리오]
보유 종목 위주. 신규 후보 시나리오는 상세표가 없으므로 생략하거나 1줄만.
...
규칙:
- 보유 종목 위주. 신규 후보는 현재가 데이터가 없으면 포함 금지.
...
[보고서 길이 원칙]
- 7~9페이지 수준 목표 (섹션 5, 6, 7 제거로 길이 단축)
...
- "신규 매수 0원"은 최대 2곳(상단 요약 + 최종 실행 플랜)에만 표시
- 최종 실행 플랜은 요약형으로만
- 신규 후보 상세표는 코드 생성 섹션에만. LLM 본문에 상세표 금지.

[보고서 실패 조건 — 하나라도 발생 시 마지막에 반드시 표시]
아래 조건이면 "❌ 보고서 실패 — 실전 매매 사용 금지"를 출력해라:
1. LLM이 신규 후보 상세표(순위/현재가/RR/손절가/목표가 컬럼 포함 표)를 생성함
2. 신규 후보 현재가 "미수집" 상태인데 RR 수치가 표시됨
3. 손절가/목표가 "미정"인데 RR 수치가 표시됨
4. 삼성전자/한국 반도체 비정상 실적 수치(50조 이상 영업이익 등)가 통과로 표시됨
5. 모델 정확도 30% 미만인데 신규 A/B 매수 제안 또는 매수금액 > 0
6. 셀트리온제약/RGTI/BEAM이 신규 후보로 표시됨
7. C등급 후보에 매수금액(0원 외)이 표시됨
8. 보유 포트폴리오 총액이 현금만으로 확정 표시됨
9. 뉴스가 있는데 가격 반영 여부/단기 영향/1~6개월 영향/행동 중 하나가 없음
10. 예상 상승률/하락률이 전부 ±10% 이내
11. TDE/LLM 충돌이 있고 미해결 상태
12. "오늘 매수"와 "오늘 금지"가 같은 종목에 충돌
13. 미국/한국 기준일이 휴장일인데 "전일 종가"로 잘못 표기
14. 단일 소스 수치를 확정값처럼 표시
15. 스크리너 JSON이 있는데 LLM이 임의 신규 후보를 생성함
16. 신규 후보 상세표가 LLM 본문에 2개 이상 출력됨

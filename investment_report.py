...
def validate_candidate_reality(candidate: dict) -> tuple:
    """신규 수익 발굴 후보의 현실성을 강제 게이트로 검증한다.

    candidate 키:
      name, ticker, current_price, stop_price, target_price, rr,
      data_source_count, is_kr, chg_1d, chg_1m, chg_3m, chg_6m,
      earnings_text  (실적/뉴스 수치 포함 문자열)

    반환: (verdict: str, issues: list[str])
    verdict: "통과" / "C관찰만" / "검증필요" / "실패/제외"
    """
    import re as _re
    issues  = []
    verdict = "통과"
    ...
    is_kr   = candidate.get("is_kr",
              ticker.endswith(".KS") or ticker.endswith(".KQ") or ticker.endswith(".KT"))
    chg_1d  = candidate.get("chg_1d")
    chg_1m  = candidate.get("chg_1m")
    chg_3m  = candidate.get("chg_3m")
    chg_6m  = candidate.get("chg_6m")
    e_text  = candidate.get("earnings_text", "")

    # ── 1. 현재가 ────────────────────────────────────────────────────────────
    if cp is None or cp <= 0:
        issues.append("현재가 미수집 — TOP 5 제외")
        return ("실패/제외", issues)

    # ── 2. 한국 주식 가격 이상 탐지 ──────────────────────────────────────────
    _kr_large = {"005930", "000660", "035420", "005490", "051910", "068270", "000270", "012330"}
    _base_ticker = ticker.replace(".KS", "").replace(".KQ", "")
    if is_kr:
        if chg_1d is not None and abs(chg_1d) >= 30:
            issues.append(f"전일대비 {chg_1d:+.0f}% — 한국 대형주 비정상 변동 (서킷브레이커 수준), 후보 제외")
            return ("실패/제외", issues)
        if chg_1m is not None and abs(chg_1m) >= 50:
            issues.append(f"1개월 {chg_1m:+.0f}% — 교차검증 필요")
            verdict = "검증필요"
        if chg_6m is not None and chg_6m >= 100 and _base_ticker in _kr_large:
            issues.append(f"6개월 {chg_6m:+.0f}% — 한국 대형주 6개월 100%+ 상승, 액면분할/단위 오류 검토 필요")
            verdict = "C관찰만" if verdict == "통과" else verdict

    # ── 3. 미국 주식 가격 이상 탐지 ──────────────────────────────────────────
    else:
        if chg_1d is not None and abs(chg_1d) >= 20:
            issues.append(f"전일대비 {chg_1d:+.0f}% — 미국 대형주 비정상, 교차검증 필요")
            verdict = "검증필요"
        if chg_1m is not None and abs(chg_1m) >= 50:
            issues.append(f"1개월 {chg_1m:+.0f}% — 교차검증 필요")
            verdict = "검증필요"

    # ── 4. 실적/뉴스 수치 검증 (비현실적 조 단위 탐지) ───────────────────────
    if e_text:
        _jo_vals = _re.findall(r'(\d+(?:\.\d+)?)\s*조', e_text)
        for _vs in _jo_vals:
            _v = float(_vs)
            if _v >= 50:
                # 삼성전자 역대 최고 연간 영업이익 43조 → 50조 이상은 비현실
                issues.append(
                    f"실적 수치 {_v:.0f}조 — 단위 오류 가능성 (분기/연간/매출/시총 혼동), "
                    "확정값 사용 금지, 교차검증 실패"
                )
                verdict = "실패/제외"
            elif _v >= 20:
                issues.append(f"실적 수치 {_v:.0f}조 — 교차검증 필요 (연간/분기/단위 불명확)")
                if verdict == "통과":
                    verdict = "검증필요"

    # ── 5. 등락률 범위 검증 ───────────────────────────────────────────────────
    if chg_3m is not None and chg_3m >= 100:
        issues.append(f"3개월 {chg_3m:+.0f}% — 교차검증 필요")
        if verdict in ("통과", "C관찰만"):
            verdict = "검증필요"
    if chg_6m is not None and chg_6m >= 150:
        issues.append(f"6개월 {chg_6m:+.0f}% — 교차검증 실패 시 TOP 5 제외")
        verdict = "실패/제외"

    # ── 6. 손절/목표 구조 ─────────────────────────────────────────────────────
    if sp is not None and cp is not None and sp >= cp:
        issues.append("손절가 >= 현재가 — A/B/C 후보 금지")
        verdict = "실패/제외"
    if tp is not None and cp is not None and tp <= cp:
        issues.append("목표가 <= 현재가 — A/B/C 후보 금지")
        verdict = "실패/제외"
    ...
    if rr is None:
        ...
        issues.append(f"RR {rr:.2f} < 1.5 — A/B 부적합")
        if verdict == "통과":
            verdict = "C관찰만"

    # ── 8. 단일소스 ──────────────────────────────────────────────────────────
    if src <= 1:
        issues.append("단일 소스 — A/B 금지, C 관찰만 허용")
        if verdict == "통과":
            verdict = "C관찰만"
...
def validate_candidate_metrics(candidate: dict) -> tuple:
    """backward-compatible wrapper → validate_candidate_reality 호출.

    반환: (is_valid: bool, issues: list[str])
    verdict '통과'만 True, 나머지는 False.
    """
...
등급 기준:
- 보유 포트폴리오 관리 A: 현재가/비중/원금/손익 모두 정상 + 충돌 없음
- 보유 포트폴리오 관리 B: 일부 추정/미수집 있지만 핵심 종목 정상
- 손실 제한 판단 A: 가격 조건 발동 종목의 수량/비중/기준가가 명확
- 신규 수익 후보 관찰 A: TOP 5 수치 검증 전부 통과 + 진입조건 4요소 충족
- 신규 수익 후보 관찰 B: TOP 5 수치 검증 통과 + 진입조건 3요소 충족
- 신규 수익 후보 관찰 C: TOP 5는 있으나 일부 단일소스/검증필요 포함
- 신규 수익 후보 관찰 D: 수치 검증 실패 후보가 TOP 5에 있거나 진입조건 2요소 이하
- 신규 매수 실행 A/B: 모델 정확도 + 데이터 검증 + RR + 매수금액 모두 충족
- 신규 매수 실행 D: 모델 정확도 30% 미만 또는 A/B 후보 0개
- 모델 정확도 30% 미만이면: 신규 매수 실행 무조건 D
...
| 우선순위 | 종목 | 현재 상태 | 오늘 액션 | 실행 수량/금액 | 조건 | 금지 | 이유 |
|----------|------|-----------|---------|------------|----|----|------|

규칙:
- 가격 조건 TRIGGERED 종목: "오늘 액션"에 즉시 재검토 행동 명시.
- 자동매도 표현 금지. 데이터 미수집을 매도 근거로 쓰지 마라.
- 축소검토이면 수량 또는 비중 목표를 표시. (예: 100~150주 축소 → 비중 25%→20%)
- 익절검토이면 수량/가격/비중 목표를 표시.
- 보유이면 보유 이유와 깨지는 조건을 표시.
- 추매금지이면 추매금지 이유를 표시.
- 데이터 미수집이면 매매 판단 금지.
- 예: 셀트리온제약 → 오늘 액션: 100~150주 축소 즉시 재검토 / 실행: 비중 25%→20% / 조건: 43,000원 이탈 시 추가 축소 / 금지: 물타기/추매 금지
...
- 삼성전자/한국 반도체 실적 수치가 조 단위로 과도하게 크면 교차검증 없이 확정값 사용 금지.
...
[섹션 5. 🎯 신규 수익 발굴 TOP 5]
신규 후보가 없더라도 검증 통과 관찰 후보 TOP 5는 반드시 출력. "후보 없음"으로 끝내지 마라.
...
데이터 검증: 통과 / C관찰만 / 검증필요 / 실패/제외

TOP 5 구성 원칙 (반드시 준수):
- 안정형: 최대 1개
- 보유추가형: 최대 1개 (보유 종목 합산 TOP 5 중 최대 2개까지만)
- 추세형: 2~3개 필수
- 이벤트형: 1~2개 포함 시도
- 현금대체형: 필요 시 1개
- 신규 종목(보유하지 않은 종목)이 최소 3개 이상 들어가야 한다.
- 보유 종목이 TOP 5 중 3개를 초과하면 안 됨.
- 검증 통과 후보 5개 미만이면 "검증 통과 후보 부족"을 명시하고 경고 표시.

후보 유형 우선순위:
한국 추세형: HD현대일렉트릭, LS ELECTRIC, 한화에어로스페이스, 현대로템, 두산에너빌리티, LG전자, SK하이닉스, 한미반도체
미국 추세형: AVGO, AMD, TSM, ASML, NVDA, PLTR, META, AMZN
안정형: VOO, QQQ, SCHD, GOOGL, MSFT, FCX, GLD
이벤트형: 정책/수주/실적/정부지원/전력인프라/방산/AI인프라/원자재 리레이팅 관련

제외 규칙:
- 셀트리온제약, RGTI, BEAM — 신규 수익 발굴 TOP 5 절대 금지. 보유관리 섹션만.
- 보유 종목이 TOP 5 중 3개 초과 → 실패.
- 데이터 미수집 종목 → TOP 5 절대 금지.
- 손절가 >= 현재가 → A/B/C 후보 금지.
- 목표가 <= 현재가 → A/B/C 후보 금지.
- 단일 소스 수치 → A/B 금지, C 관찰만.
- 수치 검증 실패("실패/제외") 후보 → TOP 5에 남기지 마라. 별도 "제외 후보" 표로 내려라.
- 삼성전자/SK하이닉스: 실적 수치가 교차검증 실패면 C관찰만 또는 TOP 5 제외.
- 네이버 비중 18% 이상이면 신규 후보 아님 — 익절/관리 후보로 표시.

진입 조건 4요소 — 반드시 3개 이상 포함:
1. 가격 조건 (예: 20일선 회복 / 전고점 돌파 / 특정 지지가격 3일 종가 안착)
2. 거래량 조건 (예: 20일 평균 거래량의 1.3배 이상 / 기관·외인 순매수)
3. 기술 조건 (예: RSI 40~65 / MACD 골든크로스 / MA20>MA60 / 52주 신고가 후 지지)
4. 뉴스/이벤트 조건 (예: 실적 가이던스 확인 / 수주·정부지원 공식 확인 / 가격·거래량 반응 확인)
- 3개 미만이면 "조건 부족 — 매수 금지" 표시.
- 과열 RSI 75 이상이면 신규 추격 금지.
- 진입 조건 예시: "$200 지지 3일 + 거래량 1.3배 + RSI 45~60 + 실적 가이던스 확인"

수치 교차검증 결과 표 (TOP 5 아래 반드시 출력):

🧪 신규 후보 수치 검증 결과

| 후보 | 가격 검증 | 등락률 검증 | 실적/뉴스 수치 검증 | 손절/목표/RR 검증 | 최종 판정 |
|------|---------|----------|------------|------------|------|

최종 판정: 통과 / C관찰만 / 검증필요 / 실패/제외
...
1. 신규 후보 TOP 5가 없음
2. TOP 5 후보 중 하나라도 현재가/진입가/손절가/목표가/RR/상승률/하락률/데이터검증 누락
3. 수치 검증 실패("실패/제외") 후보가 TOP 5에 남아 있음
4. 삼성전자/한국 반도체 비정상 실적 수치(50조 이상 영업이익 등)가 통과로 표시됨
5. 모델 정확도 30% 미만인데 신규 A/B 매수 제안 또는 매수금액 > 0
6. 셀트리온제약/RGTI/BEAM이 신규 TOP 5에 들어감
7. 현재가 미수집 종목이 A/B/C TOP 5 후보에 들어감
8. 보유 포트폴리오 총액이 현금만으로 확정 표시됨
9. 뉴스가 있는데 가격 반영 여부/단기 영향/1~6개월 영향/행동 중 하나가 없음
10. 예상 상승률/하락률이 전부 ±10% 이내
11. TDE/LLM 충돌이 있고 미해결 상태
12. 손절가 >= 현재가인데 A/B/C 후보
13. 목표가 <= 현재가인데 A/B/C 후보
14. "오늘 매수"와 "오늘 금지"가 같은 종목에 충돌
15. 미국/한국 기준일이 휴장일인데 "전일 종가"로 잘못 표기
16. TOP 5가 안정형/보유형 위주이고 신규 추세형 후보가 2개 미만
17. 진입 조건에 가격/거래량/기술/뉴스 조건 중 3개 미만 포함
18. 단일 소스 수치를 확정값처럼 표시

경고 조건 (매수 실행으로 연결되면 실패):
- 모델 정확도 낮음 / 신규 A/B 후보 0개 / 데이터 단일소스 / 보유 고위험 비중 경고 / 시장 변동성 상승

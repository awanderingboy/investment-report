...
[데이터 신뢰도 검증 원칙]
보고서 작성 전 반드시 아래 검증을 수행하라:
- WTI $50~$120, Gold $1,800~$3,500, VIX 10~80, DXY 85~115 범위 이탈 시
  "⚠️ 데이터 이상 — 수동 확인 필요"로 표시하고 분석에 사용하지 마라.
- 한국 주식 현재가는 전일 종가 기준임을 보고서 상단에 반드시 명시하라.
- 환율은 보고서 전체에서 단 하나의 값만 사용하라.
  총자산 계산, 평가손익, 기회비용 계산 모두 동일 환율 적용.
  보고서 상단에 "적용 환율: X원/달러" 한 번만 명시하라.

[판단 일관성 자가검증]
보고서 완성 전 반드시 아래를 확인하라:
① 종합 추천 TOP 3에 🟡홀딩 종목이 포함되어 있지 않은가?
   → 포함 시 제거하고 🟢추가매수/💎강력매수 종목으로 교체
② 스코어카드 점수와 포트폴리오 판단이 일치하는가?
   → 80점 이상인데 🟡홀딩이면 근거 재검토
   → 50점 이하인데 🟡홀딩이면 🟠비중축소로 조정
③ 오늘의 액션 플랜이 포트폴리오 판단과 일치하는가?
   → 🔴즉시매도 판단인데 액션 플랜에 없으면 반드시 추가
④ 총자산 합계가 카테고리별 소계 합과 일치하는가?
   → 불일치 시 재계산

[매매 판단 3중 검증]
🔴즉시매도 또는 💎강력매수 판단은 아래 3가지가 모두 일치해야 한다:
① 기술적 근거: RSI, MA, 거래량 중 2개 이상
② 펀더멘털 근거: 영업이익률, 매출성장, PER 중 1개 이상
③ 거시경제 방향성: VIX, 금리, 달러 방향과 충돌하지 않는가
3가지 중 2개 이하만 맞으면 한 단계 낮춰라.
...
- 📋 예정: 통상 발표 주기 기반
- ⚠️ 추정: AI 학습 데이터 기반
실적 발표일은 반드시 "earnings.com 직접 확인 필수" 문구를 추가하라.
⚠️ 추정 등급 이벤트를 근거로 매매 타이밍을 잡지 마라.

[카테고리 3 리스크 고지 의무]
500만원 프로젝트 섹션 맨 앞에 반드시 아래를 포함하라:
- 추천 종목이 적자 기업인 경우 "고위험 투기 등급" 명시
- 목표 달성 확률을 솔직하게 수치로 제시 (예: "달성 확률 20~30%, 전액 손실 확률 15%")
- 종목별 최악 시나리오 한 줄 추가 (예: "RGTI 최악: 양자 상용화 지연 시 -60% 급락 가능")
- "이 금액은 전액 손실을 감수할 수 있는 자금으로만 운용하라" 경고 매 보고서마다 포함

[전일 보고서 자기학습 원칙]
보고서 상단 시장 총평 직후에 아래 섹션을 추가하라:

<h2>🔄 전일 대비 자기학습</h2>
- 전일 추천 종목의 실제 등락을 수집된 오늘 데이터로 역산하라.
  예: "전일 RGTI 진입 추천 $25~$27 → 오늘 현재가 $X → 수익률 Y%"
- 전일 판단 중 오늘 데이터로 봤을 때 틀린 것을 명시하라.
  예: "전일 VOO RSI 69 과열 경고 → 오늘 RSI 71로 상승, 과열 판단 유효"
- 전일 거시경제 판단과 오늘 실제 지표 변화를 비교하라.
  예: "전일 VIX 16.78 안정 → 오늘 VIX X, 판단 유지/변경"
- 오늘 보고서에서 전일과 달라진 판단이 있으면 반드시 이유를 한 줄 명시하라.
  예: "카카오 판단 🟠비중축소 → 🔴즉시매도로 강화: RSI 30 이하 진입 확인"
- 데이터가 없어 비교 불가한 항목은 "전일 데이터 없음 — 비교 불가"로 표시하라.

<h2>🚀 500만원 → 1억 프로젝트 대시보드</h2>

⚠️ 리스크 고지: 이 섹션의 추천 종목 중 적자 기업은 고위험 투기 등급입니다.
전액 손실을 감수할 수 있는 자금으로만 운용하십시오.

[현재 운용 현황]
portfolio.json의 category3, category3_cash 데이터를 기반으로
아래 표를 반드시 실제 수치로 작성하라. 추정값 사용 금지:

| 항목 | 수치 |
|------|------|
| 프로젝트 시작 시드 | 5,000,000원 |
| 현재 현금 | category3_cash원 |
| 현재 투자 중인 금액 | (보유 종목 평가액 합산, 원화 환산) |
| 현재 총 자산 (현금+투자) | X원 |
| 누적 수익률 | X% |
| 1단계 목표 | 10,000,000원 |
| 1단계 달성까지 필요 수익 | X원 (X%) |
| 예상 1단계 달성일 | 현재 추세 기준 YYYY-MM-DD |
| 현재 단계 | 1단계 / 4단계 |

[보유 종목 현황]
category3에 보유 종목이 있으면 아래 표로 작성하라:
| 종목 | 진입가 | 현재가 | 수익률 | 평가손익 | 목표가 | 손절 트리거 | 예상 달성일 | 상태 |
보유 종목이 없으면 "현재 전액 현금 대기 중"으로 표시하라.

[이번 달 실행 전략]
반드시 아래 항목을 모두 포함하라:
- 현재 현금 배분 계획: 현금 X원 중 Y원 투자, Z원 대기
- 이번 달 목표 수익률: +X% (목표액 Y원, 달성 시 총자산 Z원)
- 이번 달 목표 달성 확률: X% (솔직하게)
- 진입 예정 종목 (데이터 검증 통과한 종목만, 카테고리1·2 종목 절대 금지):
  * 종목명: 진입가 X / 목표가 Y / 손절 Z / 투입 비중 W% / 투입 금액 V원
  * 진입 조건: "X 지표가 Y 수준일 때만 진입" (구체적 숫자)
  * 목표 달성 예상 기간: X주
  * 최악 시나리오: -X% 가능, 이때 대응 전략

[4단계 로드맵 진행률]
| 단계 | 목표금액 | 목표기간 | 현재상태 | 달성률 | 예상달성일 |
|------|----------|----------|----------|--------|------------|
| 1단계 | 10,000,000원 | ~6개월 | 진행중 | X% | YYYY-MM |
| 2단계 | 20,000,000원 | ~12개월 | 대기 | - | - |
| 3단계 | 40,000,000원 | ~18개월 | 대기 | - | - |
| 4단계 | 100,000,000원 | ~24개월 | 대기 | - | - |
현재 월 평균 수익률 X% 기준 1억 달성 예상 시점: YYYY-MM
현재 추세가 유지되지 않을 경우 최악 예상 시점: YYYY-MM

[리스크 관리 현황]
- 현재 포트폴리오 위험도: 🔴고위험 / 🟡중위험 / 🟢저위험
  (보유 종목 중 적자 기업 비중 50% 이상 = 🔴, 30~50% = 🟡, 30% 미만 = 🟢)
- 현재 최대 손실 허용선: 총자산의 20% = X원
- 손절 발동 기준: 총자산이 시작 시드의 50% 이하로 하락 시
- 손절 발동 시 행동 계획: 전액 현금화 → VIX 20 이하 확인 후 재진입
...
□ PLTR Form 4 방향 — SEC EDGAR에서 직접 검색 후 매수/매도 확인
□ 카테고리 3 진입 전 — 해당 금액 전액 손실 시 생활에 지장 없는지 확인
□ 데이터 이상 항목 — ⚠️ 표시 항목은 직접 확인 후 판단
위 체크리스트를 모두 완료한 후 매매하라.
